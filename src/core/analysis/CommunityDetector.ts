...
export class CommunityDetector {
  ...
  async detect(): Promise<Map<string, number>> {
    ...
    Logger.info(`CommunityDetector: running on ${nodes.length} nodes, ${edges.length} edges`);
    ...
    while (improved && iterations < MAX_ITER) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (const [candidateCom, kIn] of comWeights) {
          ...
          let sumDeg = 0;
          for (let j = 0; j < n; j++) {
            if (community[j] === candidateCom) sumDeg += degree[j];
          }
    ...
    Logger.info(`CommunityDetector: ${numCommunities} communities detected after ${iterations} iterations`);
