    ...
    start() {
        this.server = http.createServer((req, res) => {
            ...
            if (req.url === '/') {
                ...
                const nodes = this.store.getAllNodes().slice(0, 800);
                const edges = this.store.getAllEdges().slice(0, 1500);
...
<head>
  ...
  <style>
    ...
    #tooltip { position: absolute; background: var(--surface); border: 1px solid var(--border); padding: 8px; border-radius: 4px; pointer-events: none; opacity: 0; font-size: 12px; transition: opacity 0.2s; }
  </style>
</head>
<body>
...
<script>
  // Because we are in a normal browser, D3 will load perfectly.
  ...
  function renderGraph(nodes, edges) {
    ...
    const simulation = d3.forceSimulation(d3nodes)
      .force('link', d3.forceLink(d3links).id(d => d.id).distance(60))
